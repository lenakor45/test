...
from .models import FinancialEntry, Lesson, LessonStatus, Payment

class FinanceError(Exception): pass

def lesson_cost(hourly_rate_kopecks: int, duration_minutes: int) -> int:
    if hourly_rate_kopecks < 0 or duration_minutes < 0: raise ValueError('rate and duration must be non-negative')
...
async def add_payment(db: AsyncSession,parent_id:int,amount_kopecks:int,paid_on,comment:str='')->Payment:
    ...
    p=Payment(parent_id=parent_id,amount_kopecks=amount_kopecks,paid_on=paid_on,comment=comment); db.add(p); await db.flush(); return p

async def complete_lesson(db: AsyncSession,lesson_id:int)->int:
    lesson=(await db.scalars(select(Lesson).options(selectinload(Lesson.student)).where(Lesson.id==lesson_id).with_for_update())).one_or_none()
    if lesson is None: raise FinanceError('lesson not found')
    if lesson.status==LessonStatus.COMPLETED.value: return int(lesson.charged_kopecks or 0)
    if lesson.status not in (LessonStatus.PLANNED.value,LessonStatus.MOVED.value): raise FinanceError('only planned lessons can be completed')
    cost=lesson_cost(lesson.hourly_rate_kopecks,lesson.duration_minutes)
    lesson.status=LessonStatus.COMPLETED.value; lesson.charged_kopecks=cost
